...
ZK_USER=${ZK_USER:-"zk"}
ZK_HOME=${ZK_HOME:-"/zk"}
ZOO_DATADIR=${ZOO_DATADIR:-"/zk_data"}
ZOO_DATALOGDIR=${ZOO_DATALOGDIR:-"/zk_datalog"}
ZOO_LOG_DIR=${ZOO_LOG_DIR:-"/zk_log"}
...
ZK_HEAP_SIZE=${ZK_HEAP_SIZE:-128Mi}
ZK_MAX_CLIENT_CNXNS=${ZK_MAX_CLIENT_CNXNS:-60}
ZK_MIN_SESSION_TIMEOUT=${ZK_MIN_SESSION_TIMEOUT:- $((ZK_TICK_TIME*2))}
ZK_MAX_SESSION_TIMEOUT=${ZK_MAX_SESSION_TIMEOUT:- $((ZK_TICK_TIME*20))}
ZK_SNAP_RETAIN_COUNT=${ZK_SNAP_RETAIN_COUNT:-3}
ZK_PURGE_INTERVAL=${ZK_PURGE_INTERVAL:-0}
ID_FILE="$ZOO_DATADIR/myid"
...
function validate_env() {
    ...
    echo "ZOO_DATALOGDIR=$ZOO_DATALOGDIR"
...
function create_config() {
    ...
    echo "dataDir=$ZOO_DATADIR" >> $ZK_STATIC_CONFIG
    echo "dataLogDir=$ZOO_DATALOGDIR" >> $ZK_STATIC_CONFIG
    ...
    if [ ! -d $ZOO_DATADIR/version-2  ]; then
        ...
        echo "Created ZooKeeper data directory $ZOO_DATADIR/version-2"
    fi
    
    if [ ! -d $ZOO_DATALOGDIR/version-2  ]; then
        mkdir -p $ZOO_DATALOGDIR/version-2
        echo "Created ZooKeeper log directory $ZOO_DATALOGDIR/version-2"
    fi    
